...
# 下面指令似乎不用了 ，已經有做好 git init 自動提交的鉤子處理了
# alias   ghpi='source ~/UserScripts/git/ghpj.zsh'

# 只要是 ghr 前綴的指令，都是與遠端庫有關係的指令
alias ghrclo='gh repo clone' # 可完全取代 git clone指令，也可以直接「用戶名/倉庫名」此種參數格式下載
